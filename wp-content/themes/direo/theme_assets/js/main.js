(function ($) {
    ...
    * * * * * * * * */
    ...
    $(".sidebar-dropdown > a").on("click", function (e) {
        ...
        $(".sidebar-submenu #v-pills-active-tab, .sidebar-submenu #n-pills-active-tab").addClass("active");
    });
                ...
                if (isConfirm) {
                    ...
                    direo_ajax_handler($this, 'remove_listing', data, function (response) {
                        ...
                        if ('success' === response) {
                            ...
                            var urlHash = window.location.hash;
                            var val = $('a.direo_dash_submenu[href="'+urlHash+'"] span.badge').text();
    ...
    function direo_ajax_handler(ElementToShowLoadingIconAfter, ActionName, arg, CallBackHandler) {
        var data;
    ...
    $('#user_profile_form').on('submit', function (e) {
        ...
        direo_ajax_handler($form, 'update_user_profile', $queryString, function (response) {
            if (response.success) {
    ...
    delImgLink.on('click', function (e) {
        ...
        imgTag.attr('src', direo_localize_data.public_class_path + 'images/no-image.jpg');
    ...
    $('.atbdp_mark_as_fav').each(function () {
        $(this).on('click', function (event) {
            ...
            var fav_tooltip_success = '<span>' + direo_localize_data.i18n_text.added_favourite + '</span>';
            var fav_tooltip_warning = '<span>' + direo_localize_data.i18n_text.please_login + '</span>';
    ...
    $(".form-group .sort-rating").parent(".filter-checklist").addClass("filter-checklist-rating");
    ...
    function atbdp_user_dashboard_listing(page){
        ...
        $.post(direo_localize_data.ajaxurl, data, function(response) {
            ...
            $(".users-active__listings").empty().append(response).removeClass("atbd-loading");
            //listing table responsive fix
            if (window.innerWidth <= 991) {
                $(".dash-table tr td:not(:first-child)").hide();
                $(".dash-table tr td:first-child").each(function (id, el) {
                    $(el).on("click", function () {
                        //$(".dash-table tr td:not(:first-child)").hide();
                        $(this).parent("tr").siblings().children("td:not(:first-child)").hide();
                        $(this).parent("tr").siblings().children("td:first-child").removeClass("active");
    ...
    function atbdp_user_dashboard_needs(page) {
        ...
        $.post(direo_localize_data.ajaxurl, data, function (response) {
            // If successful Append the data into our html container
            $(".users-active__needs").empty().append(response);
            if (window.innerWidth <= 991) {
    ...
    // Handle the clicks
    $('.atbdp__user__needs li.atbd-active').live('click', function () {
        var page = $(this).attr('data-page');
