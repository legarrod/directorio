        ...
        if(!empty($display_map_info) && (!empty($display_image_map) || !empty($display_title_map) || $display_address_map) || !empty($display_direction_map)) {
            ...
            if (!empty($address)) {
                ...
                if (!empty($display_direction_map)) {
                    $html .= "<div class='osm-iw-get-location'> <a href='http://www.google.com/maps?daddr=" . $manual_lat . "," . $manual_lng . "' target='_blank'>" . __('Get Direction', 'direo-extension') . "</a> <span class='" . atbdp_icon_type() . "-arrow-right'></span></div>";
                }
            }
            $html .= "</div>";
            $html .= "</div>";
