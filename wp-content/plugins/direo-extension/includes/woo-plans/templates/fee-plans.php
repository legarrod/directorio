    ...
    <div class="atbd_plans_row">
        ...
        if ($atbdp_query->have_posts()) {
            ...
            foreach ($plans as $key => $value) {
                ...
                $num_gallery_image = esc_attr($plan_metas['num_gallery_image'][0]);
                $unl_gallery_image = esc_attr($plan_metas['unl_gallery_image'][0]);
                    ...
                    <div class="pricing pricing--1 <?php echo !empty($plan_metas['default_pln'][0]) ? 'atbd_pricing_special' : ''; ?> shadow-lg-2">
                        ...
                        <div class="pricing__price rounded">
                            <p class="pricing_value">
                                <?php
                                if (!empty($regular_price && $fm_price)) {
                                    echo '<span class="regular_price">' . $before, $regular_price, $after . '</span>';
                                }
                                ?>
                                <sup><?php echo $before; ?></sup><?php echo empty($fm_price) ? (!empty($regular_price) ? $regular_price : '0') : $fm_price; ?>
                                <sup><?php echo $after; ?></sup>
                                <small>
                                    /<?php echo ($fm_length_unl) ? $unl : $fm_length . __(' days', 'direo-extension') ?></small>
                            </p>
                        ...
                        <div class="pricing__features">
                            <ul>
                                <?php if (($plan_metas['plan_type'][0] == 'pay_per_listng') && empty(apply_filters('atbdp_plan_featured_compare', $plan_metas['_dwpp_hide_listing_featured'][0]))) { ?>
                                    <li>
                                ...
                                <?php }
                                if (empty(apply_filters('atbdp_plan_price_range_compare', $plan_metas['_dwpp_hide_price_range'][0]))) { ?>
                                    <li>
                                        <span class="fa fa-<?php echo !empty($fm_allow_price_range) ? 'check' : 'times'; ?>"> </span><?php _e('Average Price Range', 'direo-extension') ?>
                                    </li>
                                <?php }
                                if (empty(apply_filters('atbdp_plan_listing_image_compare', $plan_metas['_dwpp_hide_image'][0]))) { ?>
                                ...
                                <?php }
                                if (empty(apply_filters('atbdp_plan_business_hour_compare', $plan_metas['_dwpp_hide_business_hours'][0]))) {
                                    ...
                                    <?php } ?>
                                <?php }
                                if (empty(apply_filters('atbdp_plan_gallery_compare', $plan_metas['_dwpp_hide_image_gallery'][0]))) {
                                    ?>
                                    <li class="plan_page_gallery">
                                        <span class="fa fa-<?php if (($num_gallery_image > 0) || $unl_gallery_image) {
                                            echo 'check';
                                        } else {
                                            echo 'times';
                                        } ?>"></span><?php echo $unl_gallery_image ? '<span class="atbd_color-success">' . $unl . '</span>' . __('  Gallery Image limit', 'direo-extension') . '' : $num_gallery_image . __('  Gallery Image limit', 'direo-extension'); ?>
                                    </li>

                                <?php }
                                if (empty(apply_filters('atbdp_plan_video_compare', $plan_metas['_dwpp_hide_video'][0]))) {
                                    ?>
                                    <li>
                                ...
                                <?php }
                                if (empty(apply_filters('atbdp_plan_email_compare', $plan_metas['_dwpp_hide_email'][0]))) {
                                    ?>
                                    <li>
                                        <span class="fa fa-<?php echo !empty($fm_email) ? 'check' : 'times'; ?>"> </span><?php _e('Show Email', 'direo-extension') ?>
                                    </li>
                                <?php }
                                if (empty(apply_filters('atbdp_plan_phone_compare', $plan_metas['_dwpp_hide_phone'][0]))) {
                                    ?>
                                    <li>
                                        <span class="fa fa-<?php echo !empty($fm_phone) ? 'check' : 'times'; ?>"> </span><?php _e('Show Contact Number', 'direo-extension') ?>
                                    </li>
                                <?php }
                                if (empty(apply_filters('atbdp_plan_webLink_compare', $plan_metas['_dwpp_hide_web_link'][0]))) {
                                    ?>
                                    <li>
                                        <span class="fa fa-<?php echo !empty($fm_web_link) ? 'check' : 'times'; ?>"> </span><?php _e('Show Web Link', 'direo-extension') ?>
                                    </li>
                                <?php }
                                if (empty(apply_filters('atbdp_plan_social_network_compare', $plan_metas['_dwpp_hide_sm_link'][0]))) {
                                    ?>
                                    <li>
                                ...
                                <?php }
                                if (empty(apply_filters('atbdp_plan_faqs_compare', $plan_metas['_dwpp_hide_faqs'][0]))) {
                                    ?>
                                    <li>
                                ...
                                <?php }
                                if (empty(apply_filters('atbdp_plan_custom_field_compare', $plan_metas['_dwpp_hide_custom_fields'][0]))) {
                                    ?>
                                    <li>
